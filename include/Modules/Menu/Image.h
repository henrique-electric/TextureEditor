typedef struct _menu_image
{
    void display(Caretaker *caretaker, Originator *originator, editor_state &editor_vstate)
    {
        if (ImGui::BeginMenu("Image"))
        {
            ...
            if (ImGui::MenuItem("Binary Thresholding"))
            {
                editor_vstate.filter.binary_thresholding = true;
            }
    ...
    void binary_thresholding(editor_state &editor_vstate, Loader &loader, Caretaker *caretaker, Originator *originator, message_state &message_vstate, sdl_state &sdl_vstate)
    {

        if (editor_vstate.filter.binary_thresholding && loader.is_texture)
        { // Success on export
            Binary_Thresholding bt;

            ImGui::OpenPopup("Binary Thresholding");

            if (ImGui::BeginPopupModal("Binary Thresholding", NULL, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                if (ImGui::Button("Ok", ImVec2(60, 0)))
                {
                    editor_vstate.filter.binary_thresholding = false;
                    editor_vstate.is_processing = false;
                    ...
                    message_vstate.init = true;
                    message_vstate.message = " Applied & Exported! ( Binary Thresholding )";
